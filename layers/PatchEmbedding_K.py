...
class QuantileFlattenHead(nn.Module):
    """
    创新点 2: 多分位数输出头
    将 Encoder 的输出映射到多个分位数 (Quantiles)
    """

    def __init__(self, num_patch, d_model, pred_len, num_quantiles, dropout=0.1):
        super(QuantileFlattenHead, self).__init__()
        ...
        # 线性层输出维度扩大 num_quantiles 倍
        self.linears = nn.Linear(num_patch * d_model, pred_len * num_quantiles)
        self.dropout = nn.Dropout(dropout)
        self.flatten = nn.Flatten(start_dim=-2)

    def forward(self, x):
        # x shape: [B*N, num_patch, d_model]
        x = self.flatten(x)  # [B*N, num_patch * d_model]
        x = self.dropout(x)
        x = self.linears(x)  # [B*N, pred_len * num_quantiles]
        return x
